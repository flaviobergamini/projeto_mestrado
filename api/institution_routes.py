...
router = APIRouter(prefix="/institution", tags=["Institution"], dependencies=[Depends(require_roles("admin", "school_admin", "teacher"))])
...
@inject
async def get_institution_questions(
    storage_service: StorageService = Depends(Provide[Container.storage_service])
):
    """
    Retorna o JSON com as perguntas padrão para cadastro de instituição.
    ...
    try:
        ...
        template_path = "templates/institution_questions.json"
